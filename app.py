...
def update_conditions(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ...
        patient_conditions = Conditions_patient.query.filter_by(id=form_id).first()
        print('patient_conditions', patient_conditions)
        patient_conditions.icd10_code = request.form.get('icd10_code')
        db.session.commit()
        flash("Patient Condition Updated Successfully")
        ## then return to patient details page
        return redirect(url_for('get_patient_details', mrn=patient_conditions.mrn))
